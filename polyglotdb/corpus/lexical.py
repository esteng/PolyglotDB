...
class LexicalContext(object):
    def enrich_lexicon(self, lexicon_data, type_data = None, case_sensitive = False):
        ...
        print(type_data)
        print("time to get type_data in enrich_lexicon: {}".format(time.clock()-t60))
        if case_sensitive:
            labels = set(self.lexicon.words())
        else:
            labels = set(x.lower() for x in self.lexicon.words())
        print("time to get words in enrich_lexicon: {}".format(time.clock()-t60))
        lexicon_data = {k: v for k,v in lexicon_data.items() if k in labels}
        print("time to get lexicon_data in enrich_lexicon: {}".format(time.clock()-t60))
        self.lexicon.add_properties_once(self.word_name, lexicon_data, type_data, case_sensitive = case_sensitive)
        print("time to add properties in enrich_lexicon: {}".format(time.clock()-t60))
        lexicon_data_to_csvs(self, lexicon_data, case_sensitive = case_sensitive)
        print("time to write csvs in enrich_lexicon: {}".format(time.clock()-t60))
        import_lexicon_csvs(self, type_data, case_sensitive = case_sensitive)
        print("time to import csvs in enrich_lexicon: {}".format(time.clock()-t60))
        self.hierarchy.add_type_properties(self, self.word_name, type_data.items())
        print("time to add properties in enrich_lexicon: {}".format(time.clock()-t60))
        self.encode_hierarchy()
        print("time to encode hierarchy in enrich_lexicon: {}".format(time.clock()-t60))
