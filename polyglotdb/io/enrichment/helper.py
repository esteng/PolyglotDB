...
def parse_file(path, case_sensitive = True):
    ...
    with open(path, 'r', encoding = 'utf-8-sig') as csvfile:
        ...
        data = {}
        type_data = {}
        print("time before first for loop in parse_file: {}".format(time.clock()-t30))
        for line in reader:
            p = line[key_name]
            ...
            for i, f in enumerate(header):
                ...
                data[p][k] = v
        print("time to completion of for loops in parse_file: {}".format(time.clock()-t30))
    type_data = {k: max(v.keys(), key = lambda x: v[x]) for k, v in type_data.items()}
    print("time to return in parse_file: {}".format(time.clock()-t30))
